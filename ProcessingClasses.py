...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        for row in table:
            if row.cd_id == cd_idx:
                return row
        raise Exception('CD ID does not exist')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        """
        trk_position, trk_title, trk_artist = track_info
        try: 
            trk_position = int(trk_position)
        except:
            raise Exception('Position must be an Integer!')
        track = DC.Track(trk_position, trk_title, trk_artist)
        cd.add_track(track)
